...
function makeTestSquares (input: string[]): Square[] {
  ...
  const squares = Array.from({ length: boardSize * boardSize }, e => { return { value: ' ', state: SquareState.Empty } })
  input.forEach((v, y) => {
    [...v].forEach((u, x) => {
...
describe('tile validaton', () => {
  it('findStartWorkingTile', () => {
    const squares = new Array(boardSize * boardSize).fill({ value: ' ', state: SquareState.Empty })
...
describe('tile validaton', () => {
  ...
  it('getWordAtEdge', () => {
    ...
    expect((getWord(makeTestSquares(['', '', '', '', '', '', '', '', '', '', '', '', '', '', '           test']), 11, 14, WordDirection.Horizontal) as placedLetter[])
      .map(extractWord).join(''))
      .toEqual('TEST')
    expect((collectWords(makeTestSquares(['', '', '', '', '', '', '', '', '', '', '', '', '', '', '           test']), 11, 14, WordDirection.Horizontal))
      .map(extractWords))
      .toEqual(['TEST'])
  })

  it('collectWords', () => {
    expect((collectWords(makeTestSquares(['test']), 0, 0, WordDirection.Horizontal))
      .map(extractWords))
      .toEqual(['TEST'])
    expect((collectWords(makeTestSquares(['Test']), 1, 0, WordDirection.Horizontal))
      .map(extractWords))
      .toEqual(['TEST'])
